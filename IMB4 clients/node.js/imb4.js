...
const imbDefaultHostname = "vps17642.public.cloudvps.com"; // "localhost"
const imbDefaultPrefix = "ecodistrict"; // "nl.imb"
...
function GUIDToStringCompact(aGUID) {
	if (aGUID == undefined)
		return "";
	else
	{
		return  gc(aGUID[3])+gc(aGUID[2])+gc(aGUID[1])+gc(aGUID[0])+
				gc(aGUID[5])+gc(aGUID[4])+
				gc(aGUID[7])+gc(aGUID[6])+
				gc(aGUID[8])+gc(aGUID[9])+gc(aGUID[10])+gc(aGUID[11])+
				gc(aGUID[12])+gc(aGUID[13])+gc(aGUID[14])+gc(aGUID[15]);
	}
}
...
TIMBConnection = function (aRemoteHost, aRemotePort, aOwnerID, aOwnerName, aPrefix, aReconnectable, aPfxFile, aPassphrase, aRootCertFile) {
	...
	var hubID = empty_guid;
	var uniqueClientID = empty_guid;
	...
	this.privateEventName = function() {
		return "Clients."+GUIDToStringCompact(this.uniqueClientID)+".Private";
	}

	this.monitorEventName = function() {
